...
class AccountControllerTest {
    ...
    void testGetPlayerMatchHistory_Success() {
        ...
        // Act
        ResponseEntity<Object> response = accountController.getPlayerMatchHistory(1L);
    ...
    void testGetPlayerRank_Success() {
        // Arrange
        when(playerStatsService.getPlayerRankName(anyLong())).thenReturn("Gold");
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertNotNull(response.getBody());
        assertEquals("Gold", response.getBody().get("rankName"));
        verify(playerStatsService, times(1)).getPlayerRankName(anyLong());
    }
}
